...
fn main() {
    ...
    for path_res in paths {
        ...
        let pdu = ngap::NGAP_PDU::from_entropy::<_, DefaultEntropyScheme>(bytes.iter()).unwrap();
        let mut pdu_data = asn1_codecs::PerCodecData::new_aper();
        pdu.aper_encode(&mut pdu_data).unwrap();
        std::fs::write(format!("{}.aper", path.to_str().unwrap()), pdu_data.into_bytes()).unwrap();
    }
